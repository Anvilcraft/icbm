...
public class ProximityDetectorModePacket implements IMessage {
  ...
  public ProximityDetectorModePacket() { this(null); }
  ...
  public void fromBytes(ByteBuf buf) {
    ...
    case MIN_COORD:
      this.minCoord = Optional.of(new Vector3(clampCoord(buf.readInt()),
                                              clampCoord(buf.readInt()),
                                              clampCoord(buf.readInt())));
      break;

    case MAX_COORD:
      this.maxCoord = Optional.of(new Vector3(clampCoord(buf.readInt()),
                                              clampCoord(buf.readInt()),
                                              clampCoord(buf.readInt())));
      break;
    }
  ...
  static int clampCoord(int coord) { return Math.min(30, Math.max(0, coord)); }
