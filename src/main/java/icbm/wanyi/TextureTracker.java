...
public class TextureTracker extends TextureAtlasSprite {
  ...
  public TextureTracker() {
    super(ICBMContraption.itTracker.getUnlocalizedName().replace("item.", ""));
  }
  ...
  public void updateAnimation() {
    ...
    if (world != null) {
      ...
      if (itemStack != null) {
        if (itemStack.getItem() instanceof ItTracker) {
          Entity trackingEntity =
              ((ItTracker)itemStack.getItem())
                  .getTrackingEntity(
      ...
      angel = -((player.rotationYaw - 90.0D) * Math.PI / 180.0D -
                Math.atan2(zDifference, xDifference));
    }
    ...
    for (i = (int)((this.currentAngle / (Math.PI * 2D) + 1.0D) *
                   this.framesTextureData.size()) %
             this.framesTextureData.size();
         i < 0; i = (i + this.framesTextureData.size()) %
                    this.framesTextureData.size()) {
      ;
    }
