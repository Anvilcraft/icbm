...
public class BCamouflage extends BICBM implements IEMPBlock {
    ...
    @Override
    public void onEMP(final World world, final Vector3 position,
            final IExplosive empExplosive) {
        ...
        if (tileEntity instanceof TCamouflage) {
            ...
            world.markBlockRangeForRenderUpdate(position.intX(), position.intY(),
                    position.intZ(), position.intX(),
    ...
    @Override
    public int colorMultiplier(final IBlockAccess par1IBlockAccess, final int x,
            final int y, final int z) {
        try {
