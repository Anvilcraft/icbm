...
public class CommandAccess extends TerminalCommand {
    ...
    @Override
    public boolean processCommand(final EntityPlayer player,
            final ITerminal terminal, final String[] args) {
        if (args[0].equalsIgnoreCase("access") && args.length > 1 &&
                args[1] != null && terminal instanceof TPlatform) {
            final TPlatform platform = (TPlatform) terminal;
            final AccessLevel userAccess = terminal.getUserAccess(player.getDisplayName());
            ...
            if (args[1].equalsIgnoreCase("set") && args.length > 3 &&
                ...
                final AccessLevel playerAccess = terminal.getUserAccess(player.getDisplayName());
    ...
    public boolean canPlayerUse(final EntityPlayer var1,
        ...
        return mm.getUserAccess(var1.getDisplayName()).ordinal() >= AccessLevel.USER.ordinal() ||
    ...
    @Override
    public List<String> getCmdUses(final EntityPlayer player,
            final ISpecialAccess mm) {
