...
public class CommandTarget extends TerminalCommand {
    ...
    public boolean processCommand(final EntityPlayer player,
        ...
        if (terminal instanceof TPlatform) {
            ...
            if (turret.getTurret(false) instanceof TAutomaticTurret) {
                final TAutomaticTurret sentry = (TAutomaticTurret) turret.getTurret(false);
                if (args.length > 1) {
    ...
    public boolean canPlayerUse(final EntityPlayer var1,
        ...
        return mm.getUserAccess(var1.getDisplayName()).ordinal() >= AccessLevel.ADMIN.ordinal() ||
    ...
    public boolean canMachineUse(final ISpecialAccess mm) {
        return mm instanceof TPlatform && ((TPlatform) mm).getTurret(false) instanceof TAutomaticTurret;
    }
}
