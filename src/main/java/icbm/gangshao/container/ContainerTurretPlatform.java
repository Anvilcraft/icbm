...
public class ContainerTurretPlatform extends ContainerTerminal {
  ...
  public ContainerTurretPlatform(final InventoryPlayer inventoryPlayer,
    ...
    for (int row = 0; row < 3; ++row) {
      for (int column = 0; column < 4; ++column) {
        this.addSlotToContainer((Slot) new SlotTurret(
            (IInventory)tileEntity, column + row * 4, 8 + column * 18,
            40 + row * 18,
    ...
    for (int i = 0; i < 4; ++i) {
      this.addSlotToContainer(
          (Slot) new SlotTurret((IInventory)tileEntity, i + 12, 89 + i * 18, 77,
                                new Class[] {ITurretUpgrade.class}));
    }
    for (int row = 0; row < 3; ++row) {
      for (int slot = 0; slot < 9; ++slot) {
        this.addSlotToContainer(new Slot((IInventory)inventoryPlayer,
                                         slot + row * 9 + 9, 8 + slot * 18,
                                         97 + row * 18));
      }
    }
  ...
  public ItemStack transferStackInSlot(final EntityPlayer entityPlayer,
                                       final int slotID) {
    ItemStack var2 = null;
    ...
    if (slot != null && slot.getHasStack()) {
      ...
      if (slotID > this.tileEntity.containingItems.length - 1) {
        if (this.tileEntity.getUserAccess(entityPlayer.getDisplayName())
                .ordinal() > AccessLevel.NONE.ordinal()) {
          ...
          if (!this.mergeItemStack(
                  itemStack, this.tileEntity.containingItems.length + 27,
                  this.tileEntity.containingItems.length + 36, false)) {
            return null;
          }
        } else if (!this.mergeItemStack(
                       itemStack, this.tileEntity.containingItems.length,
                       this.tileEntity.containingItems.length + 27, false)) {
          return null;
        }
      } else if (!this.mergeItemStack(
                     itemStack, this.tileEntity.containingItems.length,
                     36 + this.tileEntity.containingItems.length, false)) {
        return null;
      }
