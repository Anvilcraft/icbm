...
public abstract class TAutomaticTurret
        extends TTurretBase implements IAutoSentry {
    ...
    public AxisAlignedBB getTargetingBox() {
        return AxisAlignedBB.getBoundingBox(
                this.xCoord - this.getDetectRange(), (double) (this.yCoord - 5),
    ...
    public boolean isValidTarget(final Entity entity) {
        if (entity != null && !entity.isDead && !entity.isEntityInvulnerable() &&
                this.getCenter().distanceTo(new Vector3(entity)) <= this.getDetectRange()) {
            final float[] rotations = super.lookHelper.getDeltaRotations(new Vector3(entity).add(
                    new Vector3(0.0, entity.getEyeHeight(), 0.0)));
            if (((rotations[1] <= super.maxPitch && rotations[1] >= super.minPitch) ||
                ...
                if (this.targetHostile && entity instanceof IMob &&
                        !this.isAir(entity)) {
                    return true;
                }
    ...
    protected boolean isAir(final Entity entity) {
        return (entity instanceof IMob && entity instanceof EntityFlying) ||
                (entity instanceof IAATarget &&
                        ((IAATarget) entity).canBeTargeted(this))
                ||
    ...
    public void sendShotToClient(final Vector3 position) {
        ICBMSentry.channel.sendToAllAround(
                PacketTurret.shot(new Vector3(this), position, super.currentRotationYaw,
                        super.currentRotationPitch),
                new TargetPoint(this.worldObj.provider.dimensionId, this.xCoord,
                        this.yCoord, this.zCoord, 40.0));
    }
    ...
    protected boolean onFire() {
        if (!this.worldObj.isRemote) {
            final ItemStack ammoStack = this.getPlatform().hasAmmunition(this.projectileType);
            if (this.getPlatform() != null && ammoStack != null) {
                ...
                if (fired && this.projectileType != ProjectileType.UNKNOWN &&
                    ...
                    if (this.getPlatform().getUpgradeCount(
                            ItPaoTaiUpgrades.TurretUpgradeType.COLLECTOR) > 0 &&
                    ...
                    if (drop && this.worldObj.rand.nextFloat() < 0.9) {
                        ...
                        final EntityItem entityShell = new EntityItem(this.worldObj, spawnPos.x, spawnPos.y,
                                spawnPos.z, ICBMSentry.bulletShell.copy());
    ...
    public double getDetectRange() {
        if (this.getPlatform() != null) {
            return Math.min(this.baseTargetRange +
