...
public class TRailgunTurret extends TPaoTaiQi implements IRedstoneReceptor {
        ...
        if (!this.worldObj.isRemote) {
            while (this.explosionDepth > 0) {
                ...
                if (objectMouseOver != null &&
                        !ICBMSentry.isProtected(this.worldObj,
                    ...
                    if (this.isAntimatter) {
                        ...
                        final AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(
                                ...
                                (double) (objectMouseOver.blockY + radius),
                                (double) (objectMouseOver.blockZ + radius));
                        final List<Entity> missilesNearby = this.worldObj.getEntitiesWithinAABB(Entity.class, bounds);
                    ...
                    final Block block = blockPosition.getBlock((IBlockAccess) this.worldObj);
                    if (block != null &&
                            block.getBlockHardness(this.worldObj, blockPosition.intX(),
                                    blockPosition.intY(),
                                    blockPosition.intZ()) != -1.0f) {
                        this.worldObj.setBlock(objectMouseOver.blockX,
                                objectMouseOver.blockY,
                                objectMouseOver.blockZ, Blocks.air, 0, 2);
                    }
                    final Entity responsibleEntity = (super.entityFake != null)
                            ? super.entityFake.riddenByEntity
                            : null;
                    this.worldObj.newExplosion(responsibleEntity, blockPosition.x,
                            blockPosition.y, blockPosition.z,
                            this.explosionSize, true, true);
                }
                --this.explosionDepth;
            }
        }
    ...
    public void playFiringSound() {
        this.worldObj.playSoundEffect((double) this.xCoord, (double) this.yCoord,
                (double) this.zCoord, "icbm.railgun", 5.0f,
                1.0f);
    }
    ...
    public void onWeaponActivated() {
        ...
        final ItemStack ammoStack = this.getPlatform().hasAmmunition(ProjectileType.RAILGUN);
        if (ammoStack != null) {
        ...
        this.worldObj.playSoundEffect(
                ...
                (double) this.zCoord, "icbm.railgun", 5.0f, 1.0f);
    }
