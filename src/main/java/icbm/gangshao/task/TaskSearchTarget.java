    ...
    protected boolean onUpdateTask() {
        ...
        if (super.tileEntity instanceof IAutoSentry) {
            ...
            if (sentry.getTarget() == null ||
                    !sentry.isValidTarget(sentry.getTarget())) {
                final AxisAlignedBB bounds = sentry.getTargetingBox();
                final List<Entity> entities = super.tileEntity.getWorldObj().getEntitiesWithinAABB(Entity.class,
                        bounds);
                Entity currentTarget = null;
                for (final Entity entity : entities) {
                    if (entity instanceof EntityPlayer) {
                        final EntityPlayer player = (EntityPlayer) entity;
                        final AccessLevel level = super.tileEntity.getPlatform().getUserAccess(
                ...
                if (currentTarget == null) {
                    ...
                    for (final Entity entity2 : entities) {
                        final double distance = super.tileEntity.getCenter().distanceTo(new Vector3(entity2));
