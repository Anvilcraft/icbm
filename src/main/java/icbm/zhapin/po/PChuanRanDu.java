...
public class PChuanRanDu extends PICBM {
  ...
  public PChuanRanDu(final int id, final boolean isBadEffect, final int color,
                     final String name) {
  ...
  @Override
  public void performEffect(final EntityLivingBase entityLiving,
                           final int amplifier) {
    if (!(entityLiving instanceof EntityZombie) &&
        !(entityLiving instanceof EntityPigZombie)) {
      entityLiving.attackEntityFrom(DamageSource.magic, 1);
    }
    if (!ICBMExplosion.shiBaoHu(((Entity)entityLiving).worldObj,
                                new Vector3((Entity)entityLiving),
                                ZhaPin.ZhaPinType.QUAN_BU, ZhaPin.chemical)) {
      final int r = 13;
      final AxisAlignedBB entitySurroundings = AxisAlignedBB.getBoundingBox(
          ((Entity)entityLiving).posX - r, ((Entity)entityLiving).posY - r,
          ((Entity)entityLiving).posZ - r, ((Entity)entityLiving).posX + r,
          ((Entity)entityLiving).posY + r, ((Entity)entityLiving).posZ + r);
      final List<EntityLivingBase> entities =
          entityLiving
              .worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
                                              entitySurroundings);
        ...
        if (entity != null) {
          if (entity instanceof EntityPig) {
            ...
            var2.setLocationAndAngles(
                ((Entity)entity).posX, ((Entity)entity).posY,
                ((Entity)entity).posZ, ((Entity)entity).rotationYaw,
