...
public class CommonProxy implements IGuiHandler {
    ...
    public void init() {
        GameRegistry.registerTileEntity(TCruiseLauncher.class,
                "ICBMCruiseLauncher");
        GameRegistry.registerTileEntity(TLauncherPlatform.class,
                "ICBMLauncherPlatform");
        GameRegistry.registerTileEntity(TLauncherControlPanel.class,
                "ICBMMissileLauncher");
        ...
        GameRegistry.registerTileEntity(TMissileCoordinator.class,
                "ICBMMissileCoordinator");
    ...
    public Object getServerGuiElement(final int ID, final EntityPlayer player,
        ...
        if (tileEntity != null) {
            switch (ID) {
                case 1: {
                    return new CXiaoFaSheQi(player.inventory, (TCruiseLauncher) tileEntity);
                }
                case 7: {
    ...
    public void spawnParticle(final String name, final World world,
            final Vector3 position, final float scale,
            final double distance) {
        this.spawnParticle(name, world, position, 0.0, 0.0, 0.0, scale, distance);
    }

    public void spawnParticle(final String name, final World world,
            final Vector3 position, final double motionX,
            final double motionY, final double motionZ,
            final float scale, final double distance) {
        this.spawnParticle(name, world, position, motionX, motionY, motionZ, 1.0f,
                1.0f, 1.0f, scale, distance);
    }
    ...
    public void spawnParticle(final String name, final World world,
            ...
            final float blue, final float scale,
            final double distance) {
    }
