...
public class BExplosives extends BICBM implements ICamouflageMaterial {
  ...
  private static byte
  determineOrientation(final World world, final int x, final int y, final int z,
    ...
    if (entityLiving != null) {
      if (MathHelper.abs((float)((Entity)entityLiving).posX - x) < 2.0f &&
        ...
        final double var5 =
            ((Entity)entityLiving).posY + 1.82 - ((Entity)entityLiving).yOffset;
        if (var5 - y > 2.0) {
          return 1;
        }
        if (y - var5 > 0.0) {
  ...
  public AxisAlignedBB getCollisionBoundingBoxFromPool(final World par1World,
    ...
    if (tileEntity != null && tileEntity instanceof TExplosive &&
        ((TExplosive)tileEntity).explosiveId == ZhaPin.sMine.getID()) {
      return AxisAlignedBB.getBoundingBox(x + this.minX, y + this.minY,
                                          z + this.minZ, x + this.maxX, y + 0.2,
                                          z + this.maxZ);
    }
  ...
  public void onBlockPlacedBy(final World world, final int x, final int y,
                              final int z, final EntityLivingBase entityLiving,
                              final ItemStack itemStack) {
    final int explosiveID =
        ((TExplosive)world.getTileEntity(x, y, z)).explosiveId;
    if (!world.isRemote &&
        ICBMExplosion.shiBaoHu(world, new Vector3(x, y, z),
                               ZhaPin.ZhaPinType.ZHA_DAN, explosiveID)) {
        ...
        VectorHelper
            .getOrientationFromSide(
                ForgeDirection.getOrientation(
                    (int)determineOrientation(world, x, y, z, entityLiving)),
    ...
    if (entityLiving != null) {
      FMLLog.fine(entityLiving.getCommandSenderName() + " placed " +
                      ZhaPin.list[explosiveID].getExplosiveName() +
                      " in: " + x + ", " + y + ", " + z + ".",
                  new Object[0]);
    }
  }
  ...
  public void onNeighborBlockChange(final World world, final int x, final int y,
    ...
    if (world.isBlockIndirectlyGettingPowered(x, y, z)) {
      yinZha(world, x, y, z, explosiveID, 0);
    } else if (block == Blocks.fire || block == Blocks.flowing_lava ||
               block == Blocks.lava) {
      yinZha(world, x, y, z, explosiveID, 2);
    }
  }

  public static void yinZha(final World world, final int x, final int y,
                            final int z, final int explosiveID,
  ...
  @Override
  public void onBlockDestroyedByExplosion(final World par1World, final int x,
  ...
  @Override
  public boolean
  onBlockActivated(final World par1World, final int x, final int y, final int z,
    ...
    if (par5EntityPlayer.getCurrentEquippedItem() != null) {
      ...
      if (this.isUsableWrench(par5EntityPlayer,
                              par5EntityPlayer.getCurrentEquippedItem(), x, y,
                              z)) {
        byte change = 3;
