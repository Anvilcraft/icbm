...
public class ExNuclear extends ExThr {
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            final ThrSheXian thread = new ThrSheXian(worldObj, position, 45, 200, explosionSource);
            thread.run();
            ...
            for (int y = 0; y < 26; ++y) {
                ...
                for (int x = -r; x < r; ++x) {
                    for (int z = -r; z < r; ++z) {
                        final double distance = MathHelper.sqrt_double((double) (x * x + z * z));
                        if (r > distance && r - 3 < distance) {
                            final Vector3 spawnPosition = Vector3.add(position, new Vector3(x * 2, (y - 2) * 2, z * 2));
                            ...
                            ICBMExplosion.proxy.spawnParticle(
                                    "smoke", worldObj, spawnPosition,
                                    xDiff * 0.3 * worldObj.rand.nextFloat(),
                                    -worldObj.rand.nextFloat(),
                                    zDiff * 0.3 * worldObj.rand.nextFloat(),
                                    (float) (distance / 45.0) * worldObj.rand.nextFloat(), 0.0f,
                                    0.0f, 8.0f, 1.2000000476837158);
                        }
                    }
                }
            }
    ...
    public void baoZhaHou(final World worldObj, final Vector3 position,
                ...
                for (final Vector3 targetPosition : thread.destroyed) {
                    ...
                    if (block == Blocks.air) {
                        ...
                        block.onBlockDestroyedByExplosion(
                                thread.world, targetPosition.intX(), targetPosition.intY(),
                                targetPosition.intZ(), (Explosion) null);
                    }
                }
            }
        ...
        this.doDamageEntities(worldObj, position, 45.0f, 200000.0f);
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.explosion", 10.0f,
                (1.0f +
                        (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2f) *
                        0.7f);
        ZhaPin.createExplosion(worldObj, position, explosionSource,
                ZhaPin.decayLand.getID());
    ...
    public void onYinZha(final World worldObj, final Vector3 position,
        ...
        if (fuseTicks % 25 == 0) {
            worldObj.playSoundEffect((double) (int) position.x, (double) (int) position.y,
                    (double) (int) position.z, "icbm.alarm", 4.0f,
                    1.0f);
        }
    }
        ...
        if (OreDictionary.getOres("ingotUranium").size() > 0) {
            RecipeHelper.addRecipe(
                    (IRecipe) new ShapedOreRecipe(
                            this.getItemStack(),
                            new Object[] { "UUU", "UEU", "UUU", 'E',
                                    ZhaPin.thermobaric.getItemStack(), 'U', "ingotUranium" }),
                    this.getUnlocalizedName(), MainBase.CONFIGURATION, true);
        } else {
            RecipeHelper.addRecipe(
                    (IRecipe) new ShapedOreRecipe(
                            this.getItemStack(), new Object[] { "EEE", "EEE", "EEE", 'E',
