...
public class ExDebilitation extends ZhaPin {
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
        ...
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.debilitation", 4.0f,
                (1.0f +
    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
            ...
            for (int i = 0; i < 200; ++i) {
                ...
                if (diDian.getMagnitude() <= radius) {
                    ...
                    ICBMExplosion.proxy.spawnParticle(
                            "smoke", explosionSource.worldObj, diDian,
                            (Math.random() - 0.5) / 2.0, (Math.random() - 0.5) / 2.0,
                            (Math.random() - 0.5) / 2.0, 1.0f, 1.0f, 1.0f, 7.0f, 8.0);
                }
            }
        }
        final AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(
                ...
                position.x + radius, position.y + radius, position.z + radius);
        final List<EntityLivingBase> allEntities = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, bounds);
        for (final EntityLivingBase entity : allEntities) {
            entity.addPotionEffect(
                    (PotionEffect) new CustomPotionEffect(Potion.confusion.id, 360, 0));
            entity.addPotionEffect((PotionEffect) new CustomPotionEffect(
                    Potion.digSlowdown.id, 1200, 0));
            entity.addPotionEffect((PotionEffect) new CustomPotionEffect(
                    Potion.moveSlowdown.id, 1200, 2));
        }
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(3),
                        new Object[] { "SSS", "WRW", "SSS", 'R', ZhaPin.repulsive.getItemStack(),
                                'W', Items.water_bucket, 'S', "dustSulfur" }),
