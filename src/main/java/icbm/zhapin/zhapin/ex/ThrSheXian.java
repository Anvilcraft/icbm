...
public class ThrSheXian extends Thread {
    ...
    public ThrSheXian(final World world, final Vector3 position,
            final int banJing, final int nengLiang,
            final Entity source) {
    ...
    public void run() {
        for (int steps = (int) Math.ceil(3.141592653589793 /
                Math.atan(1.0 / this.radius)),
                phi_n = 0; phi_n < 2 * steps; ++phi_n) {
            for (int theta_n = 0; theta_n < steps; ++theta_n) {
                final double phi = 6.283185307179586 / steps * phi_n;
                final double theta = 3.141592653589793 / steps * theta_n;
                final Vector3 delta = new Vector3(Math.sin(theta) * Math.cos(phi), Math.cos(theta),
                ...
                for (float var21 = 0.3f; power > 0.0f; power -= var21 * 0.75f * 10.0f) {
                    ...
                    final Block block = this.world.getBlock(targetPosition.intX(), targetPosition.intY(),
                            targetPosition.intZ());
                    if (block == Blocks.air) {
                        ...
                        if (block instanceof BlockLiquid) {
                            ...
                            resistance = block.getExplosionResistance(
                                    this.source, this.world, targetPosition.intX(),
