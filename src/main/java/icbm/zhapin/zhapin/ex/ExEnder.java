...
public class ExEnder extends ZhaPin {
    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
        ...
        if (worldObj.isRemote) {
            for (int r = (int) (this.getRadius() -
                    callCount / 160.0 * this.getRadius()),
                    x = -r; x < r; ++x) {
                for (int z = -r; z < r; ++z) {
                    for (int y = -r; y < r; ++y) {
                        ...
                        if (distance < r && distance > r - 1) {
                            if (targetPosition.getBlock((IBlockAccess) worldObj) == Blocks.air) {
                                if (worldObj.rand.nextFloat() < Math.max(0.001 * r, 0.01)) {
                                    final float velX = (float) ((targetPosition.x - position.x) * 0.5);
                                    final float velY = (float) ((targetPosition.y - position.y) * 0.5);
                                    final float velZ = (float) ((targetPosition.z - position.z) * 0.5);
                                    ICBMExplosion.proxy.spawnParticle("portal", worldObj,
                                            targetPosition, velX, velY,
                                            velZ, 5.0f, 1.0);
                                }
                            }
                        }
        ...
        for (final Entity entity : allEntities) {
            ...
            if (Vector3.distance(new Vector3(entity.posX, entity.posY, entity.posZ),
                    position) >= 4.0) {
                continue;
            }
            if (!explosionCreated && callCount % 5 == 0) {
                worldObj.spawnParticle("hugeexplosion", entity.posX, entity.posY,
                        entity.posZ, 0.0, 0.0, 0.0);
        ...
        worldObj.playSound(position.x, position.y, position.z, "portal.portal",
                2.0f, worldObj.rand.nextFloat() * 0.4f + 0.8f, false);
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(),
                        new Object[] { "EPE", "ETE", "EPE", 'P', Items.ender_pearl, 'E',
                                Blocks.end_stone, 'T', ZhaPin.attractive.getItemStack() }),
