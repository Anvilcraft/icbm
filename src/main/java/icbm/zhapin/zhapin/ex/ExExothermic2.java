...
public class ExExothermic2 extends ZhaPin {
    public boolean createNetherrack;

    public ExExothermic2(final String name, final int ID, final int tier) {
        super(name, ID, tier);
        this.createNetherrack = true;
        this.setFuse(1);
        MainBase.CONFIGURATION.load();
        this.createNetherrack = MainBase.CONFIGURATION
                .get("general", "Create Netherrack", this.createNetherrack)
                .getBoolean(this.createNetherrack);
                    ...
                    for (int z = 0; z < this.getRadius(); ++z) {
                        if (x == 0 || x == this.getRadius() - 1.0f || y == 0 ||
                            ...
                            for (float var18 = 0.3f; power > 0.0f; power -= var18 * 0.75f) {
                                final Vector3 targetPosition = new Vector3(var15, var16, var17);
                                final Block block = worldObj.getBlock(targetPosition.intX(),
                                        targetPosition.intY(),
                                        targetPosition.intZ());
                                if (block != Blocks.air) {
    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
        ...
        try {
            if (!worldObj.isRemote) {
                ...
                for (final Object obj : source.dataList1) {
                    ...
                    if (distanceFromCenter <= radius) {
                        ...
                        Block blockID = worldObj.getBlock((int) targetPosition.x, (int) targetPosition.y,
                                (int) targetPosition.z);
                        if (blockID == Blocks.water || blockID == Blocks.flowing_water ||
                                blockID == Blocks.ice) {
                            worldObj.setBlock((int) targetPosition.x, (int) targetPosition.y,
                                    (int) targetPosition.z, Blocks.air, 0, 2);
                        }
                        if ((blockID == Blocks.air || blockID == Blocks.snow_layer) &&
                                worldObj
                                        .getBlock((int) targetPosition.x, (int) targetPosition.y - 1,
                                                (int) targetPosition.z)
                                        .getMaterial()
                                        .isSolid()) {
                            if (worldObj.rand.nextFloat() > 0.999) {
                                worldObj.setBlock((int) targetPosition.x, (int) targetPosition.y,
                                        (int) targetPosition.z, Blocks.flowing_lava, 0,
                                        2);
                            } else {
                                worldObj.setBlock((int) targetPosition.x, (int) targetPosition.y,
                                        (int) targetPosition.z, Blocks.fire, 0, 2);
                                blockID = worldObj.getBlock((int) targetPosition.x,
                                        (int) targetPosition.y - 1,
                                        (int) targetPosition.z);
                                if (!this.createNetherrack ||
                                        (blockID != Blocks.stone && blockID != Blocks.grass &&
                                                blockID != Blocks.dirt)
                                        ||
                                        worldObj.rand.nextFloat() <= 0.75) {
                                    continue;
                                }
                                worldObj.setBlock(
                                        (int) targetPosition.x, (int) targetPosition.y - 1,
                                        (int) targetPosition.z, Blocks.netherrack, 0, 2);
                            }
                        } else {
            ...
            worldObj.playSoundEffect(
                    ...
                    "icbm.explosionfire", 6.0f,
                    (1.0f +
                            (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2f) *
                            1.0f);
        } catch (final Exception e) {
            FMLLog.severe("Conflagration explosive failed!", new Object[0]);
            e.printStackTrace();
        }
    ...
    public void gengXin(final World worldObj, final Vector3 position,
        ...
        long worldTime;
        for (worldTime = worldObj.getWorldTime(); worldTime > 23999L; worldTime -= 23999L) {
        }
        if (worldTime < 18000L) {
            worldObj.setWorldTime(worldObj.getWorldTime() + 150L);
        }
    }
