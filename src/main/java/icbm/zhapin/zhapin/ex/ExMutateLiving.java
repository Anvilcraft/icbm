    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            final AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(
                    ...
                    position.x + radius, position.y + radius, position.z + radius);
            final List<EntityLivingBase> entitiesNearby = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
                    bounds);
            for (final EntityLivingBase entity : entitiesNearby) {
                if (entity instanceof EntityPig) {
                    ...
                    newEntity.setPosition(((Entity) entity).posX, ((Entity) entity).posY,
                            ((Entity) entity).posZ);
                    entity.setDead();
                } else {
                    ...
                    newEntity2.setPosition(((Entity) entity).posX, ((Entity) entity).posY,
                            ((Entity) entity).posZ);
                    entity.setDead();
                }
            }
