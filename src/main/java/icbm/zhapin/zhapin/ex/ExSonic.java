...
public class ExSonic extends ZhaPin {
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
                ...
                for (int y = 0; y < this.getRadius(); ++y) {
                    for (int z = 0; z < this.getRadius(); ++z) {
                        if (x == 0 || x == this.getRadius() - 1.0f || y == 0 ||
                            ...
                            float power = 40.0f * (0.7f + worldObj.rand.nextFloat() * 0.6f);
        ...
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.sonicwave", 4.0f,
                (1.0f +
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(),
                        new Object[] { "@?@", "?R?", "@?@", 'R', ZhaPin.repulsive.getItemStack(),
                                '?', Blocks.noteblock, '@', "ingotBronze" }),
