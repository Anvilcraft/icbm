...
public class ExRedMatter extends ZhaPin {
    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
                                        ...
                                        if (block != null) {
                                            if (block instanceof IForceFieldBlock) {
                                                ((IForceFieldBlock) block)
                                                        .weakenForceField(worldObj, currentPos.intX(),
                                                                currentPos.intY(),
                                                                currentPos.intZ(), 50);
        ...
        for (final Entity entity2 : allEntities) {
            ...
            if (entity2 instanceof IExplosiveIgnore &&
                    ((IExplosiveIgnore) entity2)
                            .canIgnore(new ExplosionEvent(worldObj, position.x, position.y,
                                    position.z, this))) {
                continue;
            }
            ...
            if (entity2 instanceof EGravityBlock && worldObj.isRemote &&
                ...
                ICBMExplosion.proxy.spawnParticle(
                        "digging", worldObj, new Vector3(entity2), -xDifference,
                        -yDifference + 10.0, -zDifference,
                        (float) Block.getIdFromBlock(((EGravityBlock) entity2).block), 0.0f,
                        (float) ((EGravityBlock) entity2).metadata, 2.0f, 1.0);
            }
            if (Vector3.distance(
                    new Vector3(entity2.posX, entity2.posY, entity2.posZ),
                    position) >= 4.0) {
                continue;
            }
            if (!explosionCreated && callCount % 5 == 0) {
                worldObj.createExplosion(explosionSource, entity2.posX, entity2.posY,
                        entity2.posZ, 3.0f, true);
            ...
            if (entity2 instanceof EntityLiving) {
                ...
                worldObj.playSoundEffect(
                        position.x, position.y, position.z, "icbm.explosion", 7.0f,
                        (1.0f +
        ...
        if (worldObj.rand.nextInt(10) == 0) {
            worldObj.playSoundEffect(position.x + (Math.random() - 0.5) * radius,
        ...
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.redmatter", 3.0f,
                (1.0f +
                        (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2f) *
                        1.0f);
        return true;
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(), new Object[] { "AAA", "AEA", "AAA", 'E',
