...
public class ExAntimatter extends ZhaPin {
    ...
    public ExAntimatter(final String name, final int ID, final int tier) {
        ...
        this.destroyBedrock = MainBase.CONFIGURATION
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
        ...
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.antimatter", 7.0f,
                (float) (worldObj.rand.nextFloat() * 0.1 + 0.8999999761581421));
        explosionSource.posY += 5.0;
        this.doDamageEntities(worldObj, position, this.getRadius() * 2.0f,
                2.1474836E9f);
    }
    ...
    public void onYinZha(final World worldObj, final Vector3 position,
        ...
        if (fuseTicks % 25 == 0) {
            worldObj.playSoundEffect(position.x, position.y, position.z, "icbm.alarm",
                    4.0f, 1.0f);
        }
    }
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(),
                        new Object[] { "AAA", "AEA", "AAA", 'E',
                                ZhaPin.nuclear.getItemStack(), 'A', "antimatterGram" }),
