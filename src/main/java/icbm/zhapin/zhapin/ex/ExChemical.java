    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
        ...
        if (worldObj.isRemote) {
            ...
            for (int i = 0; i < 200; ++i) {
                ...
                diDian.x = Math.random() * this.getRadius() / 2.0 - this.getRadius() / 4.0f;
                diDian.y = Math.random() * this.getRadius() / 2.0 - this.getRadius() / 4.0f;
                diDian.z = Math.random() * this.getRadius() / 2.0 - this.getRadius() / 4.0f;
                ...
                if (diDian.getMagnitude() <= this.getRadius()) {
                    ...
                    ICBMExplosion.proxy.spawnParticle(
                            ...
                            (Math.random() - 0.5) / 2.0, red, green, blue, 4.0f, 8.0);
                }
            }
        }
        ...
        worldObj.playSoundEffect(
                position.x + 0.5, position.y + 0.5, position.z + 0.5, "icbm.gasleak",
                4.0f,
                (1.0f +
        ...
        if (this.getTier() == 1) {
            RecipeHelper.addRecipe(
                    (IRecipe) new ShapedOreRecipe(
                            this.getItemStack(),
                            new Object[] { "@@@", "@?@", "@@@", '@', MainBase.itDu, '?',
                                    ZhaPin.debilitation.getItemStack() }),
                    "Chemical", MainBase.CONFIGURATION, true);
        } else if (this.getTier() == 2) {
            RecipeHelper.addRecipe(
                    (IRecipe) new ShapedOreRecipe(
                            this.getItemStack(2),
                            new Object[] { " @ ", "@?@", " @ ", '?', Items.rotten_flesh, '@',
                                    ZhaPin.chemical.getItemStack() }),
                    "Contagious", MainBase.CONFIGURATION, true);
        }
    }
