...
public class ExAntiGravitational extends ZhaPin {
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
        ...
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.antigravity", 6.0f,
                (1.0f +
    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
            ...
            for (final Object obj : source.dataList1) {
                ...
                if (distance <= r && distance >= r - 2) {
                    ...
                    if (worldObj.rand.nextFloat() < 0.3 * (this.getRadius() - r)) {
                        ...
                        eFeiBlock.motionY += Math.max(0.15 * worldObj.rand.nextFloat(), 0.1);
                    }
                    --blocksToTake;
                }
            }
        ...
        final AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(
                ...
                position.x + radius, 100.0, position.z + radius);
        final List<Entity> allEntities = worldObj.getEntitiesWithinAABB(Entity.class, bounds);
        for (final Entity entity2 : allEntities) {
            if (!(entity2 instanceof EGravityBlock) &&
                    entity2.posY < 100.0 + position.y && entity2.motionY < 0.4) {
