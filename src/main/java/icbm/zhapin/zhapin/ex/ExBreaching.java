...
public class ExBreaching extends ZhaPin {
    ...
    public void doBaoZha(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            ...
            if (explosionSource instanceof IRotatable) {
                difference.modifyPositionFromSide(
                        ((IRotatable) explosionSource)
                                .getDirection((IBlockAccess) worldObj, position.intX(),
                                        position.intY(), position.intZ()));
            } else {
                difference.modifyPositionFromSide(ForgeDirection.DOWN);
            }
            worldObj.playSoundEffect(
                    position.x, position.y, position.z, "random.explode", 5.0f,
                    (1.0f +
            ...
            for (int i = 0; i < 8; ++i) {
                if (targetPosition.getBlock((IBlockAccess) worldObj) != null) {
                    if (targetPosition.getBlock((IBlockAccess) worldObj)
                            .getExplosionResistance(
                                    ...
                                    targetPosition.intY(), targetPosition.intZ(), position.x,
                                    position.y, position.z) > Blocks.obsidian.getExplosionResistance(explosionSource)) {
                        break;
                    }
                    if (targetPosition.getBlock((IBlockAccess) worldObj) instanceof IForceFieldBlock) {
                        break;
                    }
                }
                    ...
                    for (int y = 0; y < 16; ++y) {
                        for (int z = 0; z < 16; ++z) {
                            if (x == 0 || x == 15 || y == 0 || y == 15 || z == 0 || z == 15) {
                                ...
                                final double diagonalDistance = Math
                                        .sqrt(xStep * xStep + yStep * yStep + zStep * zStep);
                                ...
                                for (float var18 = 0.3f; var14 > 0.0f; var14 -= var18 * 0.75f) {
                                    ...
                                    if (var22 == Blocks.air) {
                                        var14 -= (var22.getExplosionResistance(
                                                explosionSource, worldObj, var19, var20,
                                                var21, (double) targetPosition.intX(),
                ...
                for (int var23 = blownBlocks.size() - 1; var23 >= 0; --var23) {
                    ...
                    worldObj.spawnParticle(
                            "explode", (var28 + targetPosition.x * 1.0) / 2.0,
                            (var29 + targetPosition.y * 1.0) / 2.0,
                            (var30 + targetPosition.z * 1.0) / 2.0, var31, var32, var33);
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(this.getItemStack(2),
                        new Object[] { "GCG", "GCG", "GCG", 'C',
