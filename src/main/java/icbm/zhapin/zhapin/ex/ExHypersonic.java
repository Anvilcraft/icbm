...
public class ExHypersonic extends ZhaPin {
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            ...
            for (int x = (int) (-this.getRadius() * 2.0f); x < this.getRadius() * 2.0f; ++x) {
                for (int y = (int) (-this.getRadius() * 2.0f); y < this.getRadius() * 2.0f; ++y) {
                    for (int z = (int) (-this.getRadius() * 2.0f); z < this.getRadius() * 2.0f; ++z) {
                        final Vector3 targetPosition = Vector3.add(position, new Vector3(x, y, z));
                        final Block block = worldObj.getBlock(targetPosition.intX(), targetPosition.intY(),
                                targetPosition.intZ());
                        if (block != Blocks.air) {
                            final Material material = worldObj
                                    .getBlock(targetPosition.intX(), targetPosition.intY(),
                                            targetPosition.intZ())
                                    .getMaterial();
                            if (block != Blocks.bedrock && !(block instanceof BlockLiquid) &&
                                    (block.getExplosionResistance(
                                            explosionSource, worldObj, targetPosition.intX(),
                                            targetPosition.intY(), targetPosition.intZ(),
                                            (double) position.intX(), (double) position.intY(),
                                            (double) position.intZ()) > 70.0f ||
                ...
                for (int y = 0; y < this.getRadius(); ++y) {
                    for (int z = 0; z < this.getRadius(); ++z) {
                        if (x == 0 || x == this.getRadius() - 1.0f || y == 0 ||
                            ...
                            float power = 70.0f * (0.7f + worldObj.rand.nextFloat() * 0.6f);
                            double var15 = position.x;
                            double var16 = position.y;
                            double var17 = position.z;
                            for (float var18 = 0.3f; power > 0.0f; power -= var18 * 0.75f) {
                                final Vector3 targetPosition2 = new Vector3(var15, var16, var17);
                                final Block block2 = worldObj.getBlock(targetPosition2.intX(),
                                ...
                                if (block2 != Blocks.air) {
                                    ...
                                    if (block2 instanceof BlockLiquid) {
                                        ...
                                        resistance = (block2.getExplosionResistance(
                                                explosionSource, worldObj, targetPosition2.intX(),
                                                targetPosition2.intY(), targetPosition2.intZ(),
                                                (double) position.intX(), (double) position.intY(),
        ...
        worldObj.playSoundEffect(
                position.x, position.y, position.z, "icbm.hypersonic", 6.0f,
                (1.0f +
                        (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.1f) *
                        0.9f);
    }
    ...
    public boolean doBaoZha(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            for (final Object obj : source.dataList1) {
                ...
                if (distance <= r) {
                    ...
                    final int metadata = worldObj.getBlockMetadata(targetPosition.intX(),
                            targetPosition.intY(),
                            targetPosition.intZ());
                    ...
                    if (block == ICBMExplosion.bExplosives) {
                        BExplosives.yinZha(worldObj, targetPosition.intX(),
                                targetPosition.intY(), targetPosition.intZ(),
                                ((TExplosive) worldObj.getTileEntity(
                                        targetPosition.intX(), targetPosition.intY(),
                                        targetPosition.intZ())).explosiveId,
                                1);
                    } else {
                        worldObj.setBlock(targetPosition.intX(), targetPosition.intY(),
                                targetPosition.intZ(), Blocks.air, 0, 2);
                    }
                    targetPosition.add(0.5);
                    if (worldObj.rand.nextFloat() >= 0.2 * (this.getRadius() - r)) {
                        continue;
                    }
                    final EGravityBlock entity = new EGravityBlock(worldObj, targetPosition, block, metadata);
        ...
        final AxisAlignedBB bounds = AxisAlignedBB.getBoundingBox(
                ...
                position.x + radius, position.y + radius, position.z + radius);
        final List<Entity> allEntities = worldObj.getEntitiesWithinAABB(Entity.class, bounds);
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(), new Object[] { " S ", "S S", " S ", 'S',
                                ZhaPin.sonic.getItemStack() }),
