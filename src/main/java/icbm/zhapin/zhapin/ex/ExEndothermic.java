...
public class ExEndothermic extends ZhaPin {
    ...
    public void baoZhaQian(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            final ELightBeam lightBeam = new ELightBeam(worldObj, position, 400, 0.0f, 0.3f, 0.7f);
            worldObj.spawnEntityInWorld((Entity) lightBeam);
            ((EExplosion) explosionSource).entityList.add(0, lightBeam);
            worldObj.createExplosion((Entity) null, position.x, position.y, position.z,
                    4.0f, true);
        }
    }
                            ...
                            if (dist <= r) {
                                if (dist >= r - 3) {
                                    ...
                                    final Block blockID = worldObj.getBlock(
                                            currentPos.intX(), currentPos.intY(), currentPos.intZ());
                                    if (blockID != Blocks.air && blockID != Blocks.bedrock) {
                                        if (blockID != Blocks.obsidian) {
                                            ...
                                            if (worldObj.rand.nextInt(3) > 0) {
                                                worldObj.setBlock(currentPos.intX(), currentPos.intY(),
                                                        currentPos.intZ(), Blocks.air, 0, 2);
                                                currentPos.add(0.5);
                                                final EGravityBlock entity = new EGravityBlock(
                                                        worldObj, currentPos, blockID, metadata);
                                                worldObj.spawnEntityInWorld((Entity) entity);
                                                gravityBlocks.add(entity);
                                                entity.pitchChange = 50.0f * worldObj.rand.nextFloat();
                                            }
                                        }
                                    }
                                }
            ...
            for (final Entity unspecifiedEntity : gravityBlocks) {
                if (unspecifiedEntity instanceof EGravityBlock) {
                    ...
                    if (xDifference > 4.0) {
                        ...
                        eFeiBlock.motionX += (r2 - xDifference) * -0.02 * worldObj.rand.nextFloat();
                    }
                    ...
                    if (zDifference > 4.0) {
                        ...
                        eFeiBlock4.motionZ += (r2 - zDifference) * -0.02 * worldObj.rand.nextFloat();
                    }
                    final EGravityBlock eFeiBlock5 = entity2;
                    eFeiBlock5.yawChange += 3.0f * worldObj.rand.nextFloat();
                }
            }
            worldObj.playSoundEffect(position.x, position.y, position.z,
                    "icbm.redmatter", 4.0f, 0.8f);
            return callCount <= 35;
        }
        return false;
    }
    ...
    public void baoZhaHou(final World worldObj, final Vector3 position,
        ...
        if (!worldObj.isRemote) {
            ...
            if (this.canFocusBeam(worldObj, position)) {
                ...
                final List<EntityLivingBase> livingEntities = worldObj.getEntitiesWithinAABB(
                        EntityLivingBase.class,
                        AxisAlignedBB.getBoundingBox(position.x - this.getRadius(),
                ...
                for (final EntityLivingBase entity2 : livingEntities) {
                    entity2.addPotionEffect((PotionEffect) new CustomPotionEffect(
                            PDongShang.INSTANCE.getId(), 1200, 1, null));
                    entity2.addPotionEffect(
                            new PotionEffect(Potion.confusion.id, 200, 2));
                    entity2.addPotionEffect(
                            new PotionEffect(Potion.digSlowdown.id, 2400, 2));
                    entity2.addPotionEffect(
                            new PotionEffect(Potion.moveSlowdown.id, 2400, 4));
                }
                ZhaPin.createExplosion(worldObj, position, explosionSource,
                        ZhaPin.endothermic2.getID());
            }
        }
    }
    ...
    public void init() {
        RecipeHelper.addRecipe(
                (IRecipe) new ShapedOreRecipe(
                        this.getItemStack(),
                        new Object[] { "?!?", "!@!", "?!?", '@', ZhaPin.attractive.getItemStack(),
                                '?', Blocks.ice, '!', Blocks.snow }),
    ...
    public boolean canFocusBeam(final World worldObj, final Vector3 position) {
        ...
        return worldTime > 12000L &&
                worldObj.canBlockSeeTheSky(position.intX(), position.intY() + 1,
                        position.intZ());
    }
