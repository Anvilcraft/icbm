...
public class RYinDaoQi extends TileEntitySpecialRenderer {
    ...
    public void renderModelAt(final TMissileCoordinator tileEntity, final double x,
        ...
        final List<EntityPlayer> players = tileEntity.getWorldObj().getEntitiesWithinAABB(
                EntityPlayer.class,
                AxisAlignedBB.getBoundingBox((double) (tileEntity.xCoord - radius),
        ...
        if (players.size() > 0) {
            this.bindTexture(new ResourceLocation(
                    "icbm", "textures/models/missile_coordinator_on.png"));
        } else {
            this.bindTexture(new ResourceLocation(
        ...
        switch (tileEntity
                .getDirection((IBlockAccess) tileEntity.getWorldObj(), (int) x,
                        (int) y, (int) z)
                .ordinal()) {
            case 2: {
