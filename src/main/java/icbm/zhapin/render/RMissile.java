...
public class RMissile extends Render {
  ...
  public RMissile(final float f) { super.shadowSize = f; }

  @Override
  public void doRender(final Entity entity, final double x, final double y,
                       final double z, final float f, final float f1) {
    ...
    GL11.glRotatef(
        entityMissile.prevRotationPitch +
            (entityMissile.rotationPitch - entityMissile.prevRotationPitch) *
                f1 +
            90.0f,
        0.0f, 0.0f, 1.0f);
    ...
    if (entityMissile.missileId >= 100) {
      ...
      this.bindTexture(new ResourceLocation(
          "icbm",
          "textures/models/missile_" +
              ZhaPin.list[entityMissile.missileId].getUnlocalizedName() +
              ".png"));
...
protected ResourceLocation getEntityTexture(Entity p_110775_1_) {
  ...
  static {
    MODELS =
        new MICBM[] {new MMYaSuo(),    new MMXiaoQunDan(), new MMHuo(),
    ...
    RMissile.SPECIAL_MODELS =
        new MICBM[] {new MMYaSuo(), new MMZhuiZhong(), new MMFanDan(),
                     new MMFenZiDan(), new MMFenZiDan()};
  }
}
