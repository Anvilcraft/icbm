    ...
    public ItemStack onItemRightClick(final ItemStack itemStack,
        ...
        if (!world.isRemote && this.getJoules(itemStack) >= 5000.0) {
            for (int i = 0; i < player.inventory.getSizeInventory(); ++i) {
                ...
                if (inventoryStack != null && inventoryStack.getItem() instanceof ItMissile) {
                    ...
                    if (daoDan != null &&
                            !ICBMExplosion.shiBaoHu(world, new Vector3((Entity) player),
                                    ZhaPin.ZhaPinType.DAO_DAN, haoMa)) {
                        if (daoDan.getTier() <= 2 && daoDan.isCruise()) {
                            final Vector3 diDian = Vector3.add(new Vector3((Entity) player),
                                    new Vector3(0.0, 0.5, 0.0));
                            final Vector3 kan = new Vector3(player.getLook(1.0f));
                            final Vector3 kaiShiDiDian = Vector3.add(diDian, Vector3.multiply(kan, 1.1));
                            final Vector3 muBiao = Vector3.add(diDian, Vector3.multiply(kan, 100.0));
                            final EMissile eDaoDan = new EMissile(
                                    world, kaiShiDiDian, daoDan.getID(),
                                    ((Entity) player).rotationYaw, ((Entity) player).rotationPitch);
