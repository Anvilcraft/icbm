...
public class ItRemoteDetonator extends ItElectricICBM {
    ...
    public void addInformation(final ItemStack itemStack,
        ...
        if (this.nengZha(
                coord.getTileEntity((IBlockAccess) ((Entity) player).worldObj))) {
            par3List.add("\uaa74Linked Explosive:");
            par3List.add("X: " + (int) coord.x + ", Y: " + (int) coord.y +
                    ", Z: " + (int) coord.z);
        } else {
    ...
    public ItemStack onItemRightClick(final ItemStack itemStack,
        ...
        if (world.isRemote) {
            if (!entityPlayer.isSneaking()) {
                final MovingObjectPosition objectMouseOver = entityPlayer.rayTrace(100.0, 1.0f);
                if (objectMouseOver != null &&
                    ...
                    final Block blockID = world.getBlock(objectMouseOver.blockX, objectMouseOver.blockY,
                            objectMouseOver.blockZ);
                    final TileEntity tile = world.getTileEntity(objectMouseOver.blockX,
                    ...
                    if (tile != null && tile instanceof TExplosive) {
                        ...
                        if (this.nengZha(tileEntity)) {
                            if (this.getJoules(itemStack) > 1500.0) {
                                ...
                                //        PacketManager.getPacket("ICBM|E", tileEntity, 2));
                                return itemStack;
                            }
                            entityPlayer.addChatMessage(
                                    new ChatComponentText("Remote out of electricity!"));
                ...
                final TileEntity tileEntity2 = this.getSavedCoord(itemStack).getTileEntity((IBlockAccess) world);
                ...
                entityPlayer.addChatMessage(new ChatComponentText("Remote out of electricity!"));
            }
        }
        return itemStack;
    }
    ...
    public void setSavedCoords(final ItemStack itemStack,
    ...
    public Vector3 getSavedCoord(final ItemStack par1ItemStack) {
        ...
        return new Vector3(par1ItemStack.stackTagCompound.getInteger("x"),
