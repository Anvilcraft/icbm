...
public class ItLaserDesignator
        extends ItElectricICBM implements IItemFrequency {
    ...
    public void setLauncherCountDown(final ItemStack par1ItemStack,
            final int value) {
        if (par1ItemStack.stackTagCompound == null) {
    ...
    public void onUpdate(final ItemStack par1ItemStack, final World par2World,
            final Entity par3Entity, final int par4,
            final boolean par5) {
        ...
        if (!par2World.isRemote) {
            ...
            if (this.getLauncherCountDown(par1ItemStack) > 0 ||
                ...
                final Vector3 position = new Vector3(par3Entity.posX, par3Entity.posY, par3Entity.posZ);
                final List<TLauncherController> launchers = LauncherManager.launchersInArea(
                        new Vector2(position.x - ItLaserDesignator.BAN_JING,
                                position.z - ItLaserDesignator.BAN_JING),
                        new Vector2(position.x + ItLaserDesignator.BAN_JING,
                                position.z + ItLaserDesignator.BAN_JING));
                for (final TLauncherController missileLauncher : launchers) {
                    if (missileLauncher != null &&
                            missileLauncher.getFrequency() == this.getFrequency(par1ItemStack) &&
            ...
            if (this.getLauncherCountDown(par1ItemStack) > 0 &&
                    ...
                    ((EntityPlayer) par3Entity)
                            .addChatMessage(new ChatComponentText(
                                    "Calling air strike in: " +
                                            (int) Math.floor(this.getLauncherCountDown(par1ItemStack) /
                                                    20)));
                }
                ...
                this.setLauncherCountDown(par1ItemStack,
                        this.getLauncherCountDown(par1ItemStack) - 1);
            }
            if (this.getLauncherCount(par1ItemStack) > 0 &&
                ...
                if (this.getLauncherDelay(par1ItemStack) % 40 == 0) {
                    connectedLaunchers.get(this.getLauncherCount(par1ItemStack) - 1)
                            .launch();
                    this.setLauncherCount(par1ItemStack,
                            this.getLauncherCount(par1ItemStack) - 1);
                }
                if (this.getLauncherCount(par1ItemStack) == 0) {
                    this.setLauncherDelay(par1ItemStack, 0);
                    connectedLaunchers.clear();
                }
                this.setLauncherDelay(par1ItemStack,
                        this.getLauncherDelay(par1ItemStack) + 1);
            }
        }
    }
    ...
    public boolean onItemUse(final ItemStack par1ItemStack,
            final EntityPlayer par2EntityPlayer,
            final World par3World, final int x, final int y,
        ...
        if (!par3World.isRemote) {
            ...
            if (tileEntity != null && tileEntity instanceof TLauncherController) {
                final TLauncherController missileLauncher = (TLauncherController) tileEntity;
                if (missileLauncher.getFrequency() > 0) {
                    ...
                    par2EntityPlayer.addChatMessage(
                            new ChatComponentText("Laser designator frequency Set: " +
    ...
    public ItemStack onItemRightClick(final ItemStack par1ItemStack,
        ...
        if (par2World.isRemote) {
            ...
            if (objectMouseOver != null &&
                ...
                final Block block = par2World.getBlock(objectMouseOver.blockX, objectMouseOver.blockY,
                        objectMouseOver.blockZ);
                final int blockMetadata = par2World.getBlockMetadata(
                        objectMouseOver.blockX, objectMouseOver.blockY,
                        objectMouseOver.blockZ);
                if (this.getLauncherCountDown(par1ItemStack) > 0) {
                ...
                if (airStrikeFreq > 0) {
                    if (this.getJoules(par1ItemStack) > 6000.0) {
                        final Vector3 position = new Vector3(((Entity) par3EntityPlayer).posX,
                        ...
                        for (final TLauncherController missileLauncher : launchers) {
                            if (missileLauncher != null &&
                                ...
                                if (missileLauncher instanceof TCruiseLauncher) {
                                    ...
                                    missileLauncher.setTarget(
                                            new Vector3(objectMouseOver.blockX, previousY,
                                                    objectMouseOver.blockZ));
                                    // TODO: WTF
                                    // PacketDispatcher.sendPacketToServer(PacketManager.getPacket(
                                    // "ICBM", missileLauncher, 2,
                        ...
                        if (doAirStrike && this.getLauncherCountDown(par1ItemStack) >= 0) {
                            ICBMExplosion.channel.sendToServer(new ItemUsePacket(
                                    ...
                                    new Vector3(objectMouseOver.blockX, objectMouseOver.blockY,
                                            objectMouseOver.blockZ)));
                            par3EntityPlayer.addChatMessage(
                                    new ChatComponentText("Calling air strike into designated position!"));
                        }
                    } else {
                        par3EntityPlayer.addChatMessage(
                    ...
                    par3EntityPlayer.addChatMessage(new ChatComponentText("Laser designator frequency not set!"));
                }
            }
        }
        return par1ItemStack;
