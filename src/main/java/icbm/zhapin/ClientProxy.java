...
public class ClientProxy extends CommonProxy {
    ...
    public void init() {
        ...
        MinecraftForgeClient.registerItemRenderer(ICBMExplosion.itDaoDan,
                (IItemRenderer) new RItDaoDan());
        MinecraftForgeClient.registerItemRenderer(ICBMExplosion.itTeBieDaoDan,
                (IItemRenderer) new RItDaoDan());
        RenderingRegistry.registerBlockHandler(
                (ISimpleBlockRenderingHandler) new RHZhaPin());
        RenderingRegistry.registerBlockHandler(
                (ISimpleBlockRenderingHandler) new RHJiQi());
        RenderingRegistry.registerEntityRenderingHandler(EExplosive.class,
                (Render) new REZhaDan());
        RenderingRegistry.registerEntityRenderingHandler(
                EMissile.class, (Render) new RMissile(0.5f));
        RenderingRegistry.registerEntityRenderingHandler(EExplosion.class,
                (Render) new RZhaPin());
        RenderingRegistry.registerEntityRenderingHandler(EGravityBlock.class,
                (Render) new RFeiBlock());
        RenderingRegistry.registerEntityRenderingHandler(ELightBeam.class,
                (Render) new RGuangBang());
        ...
        ClientRegistry.bindTileEntitySpecialRenderer(
                TLauncherControlPanel.class, (TileEntitySpecialRenderer) new RLauncherControlPanel());
        ClientRegistry.bindTileEntitySpecialRenderer(
    ...
    @Override
    public void spawnParticle(final String name, final World world, final Vector3 position,
            final double motionX, final double motionY,
            final double motionZ, final float red, final float green,
            final float blue, final float scale, final double distance) {
        EntityFX fx = null;
        if (name == "smoke") {
            fx = (EntityFX) new FXSmoke(world, position, red, green, blue, scale,
                    distance);
            ...
            fx = (EntityFX) new FXPortal(world, position, red, green, blue, scale,
                    distance);
            ...
            fx = (EntityFX) new EntityDiggingFX(
                    world, position.x, position.y, position.z, motionX, motionY, motionZ,
                    Block.getBlockById((int) red), 0, (int) green);
