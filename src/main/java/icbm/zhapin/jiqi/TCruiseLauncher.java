...
public class TCruiseLauncher
    extends TLauncherController implements IBlockActivate, IInventory, ILauncherContainer {
  ...
  public void updateEntity() {
    ...
    if (!this.isDisabled()) {
      this.onReceive(ElectricityPack.getFromWatts(
          ElectricItemHelper.dechargeItem(this.containingItems[1],
  ...
  public void setMissile() {
    if (this.containingItems[0] != null && this.containingItems[0].getItem()
                                                   instanceof ItMissile) {
      ...
      if (!ICBMExplosion.shiBaoHu(this.worldObj, new Vector3(this),
        ...
        if (this.missile == null && MissileBase.list[haoMa].isCruise() &&
          ...
          this.missile = new EMissile(this.worldObj, startingPosition,
                                      new Vector3(this), haoMa);
  ...
  private float getPitchFromTarget() {
    ...
    return (float)Math.toDegrees(
        Math.atan((super.target.y - (this.yCoord + 0.5f)) / distance));
  }
  ...
  public boolean isTooClose(final Vector3 target) {
    return Vector3.distance(new Vector3(this.xCoord, 0.0, this.zCoord),
                            new Vector3(target.x, 0.0, target.z)) < 8.0;
  }
  ...
  public boolean onActivated(final EntityPlayer entityPlayer) {
    ...
    entityPlayer.openGui((Object)ICBMExplosion.instance, 1, this.worldObj,
