...
public class TEmpTower extends TileEntityUniversalStorable
        implements IMultiBlock, IRedstoneReceptor {
        ...
        if (!this.isDisabled()) {
            if (super.ticks % 20L == 0L && this.getJoules() > 0.0) {
                this.worldObj.playSoundEffect(
                        ...
                        "icbm.machinehum", 0.5f,
                        (float) (0.8500000238418579 * this.getJoules() /
                                this.getMaxJoules()));
            }
            this.xuanZhuanLu = (float) (Math.pow(this.getJoules() / this.getMaxJoules(), 2.0) * 0.5);
        ...
        if (this.getJoules() >= this.getMaxJoules()) {
            if (this.holzOhJa == 0 || this.holzOhJa == 1) {
                ZhaPin.empSignal.doBaoZha(
                        this.worldObj, new Vector3(this.xCoord, this.yCoord, this.zCoord),
                        null, this.radius, -1);
            }
    ...
    public boolean onActivated(final EntityPlayer entityPlayer) {
        entityPlayer.openGui((Object) ICBMExplosion.instance, 6, this.worldObj,
    ...
    public void onCreate(final Vector3 position) {
        MainBase.bJia.makeFakeBlock(
                this.worldObj, Vector3.add(position, new Vector3(0.0, 1.0, 0.0)),
                new Vector3(this));
    }
