...
public class DNuclearCluster extends DCluster {
    ...
    if (missileObj.motionY < -0.5) {
      if (missileObj.missileCount < 4) {
        if (!missileObj.worldObj.isRemote) {
          ...
          final EMissile clusterMissile =
              new EMissile(missileObj.worldObj, position,
                          new Vector3(missileObj), ZhaPin.nuclear.getID());
          ...
          clusterMissile.launch(Vector3.add(
              missileObj.target,
              new Vector3((missileObj.missileCount - 2) * Math.random() * 30.0,
                          (missileObj.missileCount - 2) * Math.random() * 30.0,
                          (missileObj.missileCount - 2) * Math.random() *
                              30.0)));
        }
  ...
  public void onExplode(final EMissile missileObj) {
    ZhaPin.createExplosion(missileObj.worldObj, new Vector3(missileObj),
                           missileObj, ZhaPin.nuclear.getID());
  }
