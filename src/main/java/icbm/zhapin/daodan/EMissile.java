...
public class EMissile extends Entity implements IMissileLockable,
  ...
  public EMissile(final World par1World, final Vector3 diDian,
                  final Vector3 faSheQiDiDian, final int missileId) {
    this(par1World);
  ...
  public EMissile(final World par1World, final Vector3 diDian, final int haoMa,
  ...
  public String getCommandSenderName() {
    if (this.missileId >= 100) {
      return TranslationHelper.getLocal(
          "icbm.missile." +
          MissileBase.list[this.missileId].getUnlocalizedName() + ".name");
    }
    return TranslationHelper.getLocal(
        "icbm.missile." + ZhaPin.list[this.missileId].getUnlocalizedName() +
        ".name");
  }
  ...
  public void launch(final Vector3 target) {
    ...
    this.flyingTicks = 0;
    this.jiSuan();
    super.worldObj.playSoundAtEntity((Entity)this, "icbm.missilelaunch", 4.0f,
                                     (1.0f + (super.worldObj.rand.nextFloat() -
                                              super.worldObj.rand.nextFloat()) *
                                                 0.2f) *
                                         0.7f);
    RadarRegistry.register(this);
    MainBase.LOGGER.info("Launching " + this.getCommandSenderName() + " from " +
                         this.origin.intX() + ", " + this.origin.intY() + ", " +
                         this.origin.intZ() + " to " + this.target.intX() +
                         ", " + this.target.intY() + ", " + this.target.intZ());
  }
  ...
  public void entityInit() {
    ...
    this.daoDanInit(ForgeChunkManager.requestTicket(
        (Object)ICBMExplosion.instance, super.worldObj,
  ...
  public void updateLoadChunk(final int newChunkX, final int newChunkZ) {
    if (!super.worldObj.isRemote && MainBase.ZAI_KUAI &&
  ...
  public void onUpdate() {
    ...
    if (!super.worldObj.isRemote &&
        ICBMExplosion.shiBaoHu(super.worldObj, new Vector3(this),
                              ZhaPin.ZhaPinType.DAO_DAN, this.missileId)) {
    ...
    if (this.flyingTicks >= 0) {
      ...
      if (!super.worldObj.isRemote) {
        if (this.missileType == MissileType.SMALL_MISSILE ||
          ...
          if (this.flyingTicks == 0 && this.smallMissileMotion != null) {
            this.smallMissileMotion =
                new Vector3(this.xDiff / (this.flightDuration * 0.3),
          ...
          super.rotationPitch =
              (float)(Math.atan(super.motionY /
                                Math.sqrt(super.motionX * super.motionX +
          ...
          if (this.explosionHeight > 0 && super.motionY < 0.0) {
            final Block blockBelow = super.worldObj.getBlock(
                (int)super.posX, (int)super.posY - this.explosionHeight,
                (int)super.posZ);
      ...
      if (launcher != null) {
        ...
        if (launcher instanceof TCruiseLauncher) {
          ...
          super.noClip = true;
          if (super.worldObj.isRemote) {
            super.rotationYaw = -((TCruiseLauncher)launcher).rotationYaw + 90.0f;
  ...
  private void spawnMissileSmoke() {
    if (super.worldObj.isRemote) {
      ...
      final double dH =
          Math.cos(Math.toRadians(super.rotationPitch)) * distance;
      delta.x = Math.sin(Math.toRadians(super.rotationYaw)) * dH;
      delta.z = Math.cos(Math.toRadians(super.rotationYaw)) * dH;
      position.add(delta);
      super.worldObj.spawnParticle("flame", position.x, position.y, position.z,
                                   0.0, 0.0, 0.0);
      ICBMExplosion.proxy.spawnParticle("missile_smoke", super.worldObj,
                                       position, 4.0f, 2.0);
  ...
  public void setNormalExplode() { this.setNormalExplode = true; }

  public void setExplode() { this.setExplode = true; }
  ...
  public void explode() {
    try {
      if (!this.exploded) {
        if (this.missileId == 0) {
          if (!super.worldObj.isRemote) {
            super.worldObj.createExplosion((Entity)this, super.posX, super.posY,
                                           super.posZ, 5.0f, true);
          }
        } else {
          MissileBase.list[this.missileId].onExplode(this);
        }
        this.exploded = true;
        MainBase.LOGGER.info(this.getCommandSenderName() + " exploded in " +
                             (int)super.posX + ", " + (int)super.posY + ", " +
                             (int)super.posZ);
      }
      this.setDead();
    } catch (final Exception e) {
  ...
  public void destroyCraft() { this.normalExplode(); }

  public int doDamage(final int damage) { return -1; }
