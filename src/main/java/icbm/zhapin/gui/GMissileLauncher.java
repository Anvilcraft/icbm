...
public class GMissileLauncher extends GuiBase {
    ...
    public GMissileLauncher(
            final TLauncherControlPanel par2ICBMTileEntityMissileLauncher) {
    ...
    public void keyTyped(final char par1, final int par2) {
        ...
        try {
            final Vector3 newTarget = new Vector3(Integer.parseInt(this.tFX.getText()),
        ...
        try {
            final short newFrequency = (short) Math.max(Short.parseShort(this.tFFreq.getText()), 0);
        ...
        try {
            final short newGaoDu = (short) Math.max(
                    Math.min(Short.parseShort(this.tFGaoDu.getText()), 99), 3);
    ...
    public void mouseClicked(final int par1, final int par2, final int par3) {
        ...
        this.tFX.mouseClicked(par1 - this.containerWidth,
                par2 - this.containerHeight, par3);
        this.tFZ.mouseClicked(par1 - this.containerWidth,
                par2 - this.containerHeight, par3);
        if (this.tileEntity.getTier() >= 1) {
            this.tFY.mouseClicked(par1 - this.containerWidth,
                    par2 - this.containerHeight, par3);
            this.tFGaoDu.mouseClicked(par1 - this.containerWidth,
                    par2 - this.containerHeight, par3);
            if (this.tileEntity.getTier() > 1) {
                this.tFFreq.mouseClicked(par1 - this.containerWidth,
                        par2 - this.containerHeight, par3);
            }
        }
    }
    ...
    @Override
    public void drawForegroundLayer(final int var2, final int var3,
            final float var1) {
        ...
        if (this.tileEntity.faSheDi != null &&
                this.tileEntity.faSheDi.jiaZi != null) {
            inaccuracy = this.tileEntity.faSheDi.jiaZi.getInaccuracy();
        }
        this.fontRendererObj.drawString("Inaccuracy: " + inaccuracy + " blocks", 12,
                113, 4210752);
        this.fontRendererObj.drawString("Status: " + this.tileEntity.getStatus(),
                12, 125, 4210752);
