 ...
 * @see dan200.computercraft.api.ComputerCraftAPI#registerTurtleUpgrade(
...
public interface ITurtleUpgrade {
     ...
     * for the lifetime of the turtle, will have update() called once-per-tick, and will
     * be attach'd detach'd and have methods called in the same manner as a Computer
     * peripheral.
     *
    ...
    /**
     * Will only be called for Tool turtle. Called when turtle.dig() or turtle.attack() is
     * called by the turtle, and the tool is required to do some work.
     * @param turtle Access to the turtle that the tool resides on.
     * @param side Which side of the turtle (left or right) the tool resides on.
     * @param verb Which action (dig or attack) the turtle is being called on to perform.
     * @param direction Which world direction the action should be performed in, relative
     *     to the turtles
     * position. This will either be up, down, or the direction the turtle is facing,
     * depending on whether dig, digUp or digDown was called.
     * @return Whether the turtle was able to perform the action, and hence whether the
     *     turtle.dig()
     * or turtle.attack() lua method should return true. If true is returned, the tool
     * will perform a swinging animation. You may return null if this turtle is a
     * Peripheral and this method is not expected to be called.
     */
    public TurtleCommandResult
    useTool(ITurtleAccess turtle, TurtleSide side, TurtleVerb verb, int direction);

    /**
     * Called to obtain the IIcon to be used when rendering a turtle peripheral. Needs to
     * be a "common" type IIcon for now, as there is no way to determine which texture
     * sheet an IIcon is from by the IIcon itself.
