...
public final class ComputerCraftAPI {
    ...
    /**
     * Creates a numbered directory in a subfolder of the save directory for a given
     * world, and returns that number.<br> Use in conjuction with createSaveDirMount() to
     * create a unique place for your peripherals or media items to store files.<br>
     * @param world The world for which the save dir should be created. This should be the
     *     serverside world object.
     * @param parentSubPath The folder path within the save directory where the new
     *     directory should be created. eg: "computercraft/disk"
     * @return The numerical value of the name of the new folder, or -1 if the folder
     *     could not be created for some reason.<br>
     * eg: if createUniqueNumberedSaveDir( world, "computer/disk" ) was called returns 42,
     * then "computer/disk/42" is now available for writing.
    ...
    public static int createUniqueNumberedSaveDir(World world, String parentSubPath) {
        ...
        if (computerCraft_createUniqueNumberedSaveDir != null) {
            try {
                return (Integer) computerCraft_createUniqueNumberedSaveDir.invoke(
                    null, world, parentSubPath
                );
            } catch (Exception e) {
                // It failed
            }
    ...
    /**
     * Creates a file system mount that maps to a subfolder of the save directory for a
     * given world, and returns it.<br> Use in conjuction with IComputerAccess.mount() or
     * IComputerAccess.mountWritable() to mount a folder from the users save directory
     * onto a computers file system.<br>
     * @param world The world for which the save dir can be found. This should be the
     *     serverside world object.
     * @param subPath The folder path within the save directory that the mount should map
     *     to. eg: "computer/disk/42".<br>
     * Use createUniqueNumberedSaveDir() to create a new numbered folder to use.
     * @param capacity The ammount of data that can be stored in the directory before it
     *     fills up, in bytes.
     * @return The mount, or null if it could be created for some reason. Use
     *     IComputerAccess.mount() or IComputerAccess.mountWritable()
     ...
     * @see IWritableMount
     */
    public static IWritableMount
    createSaveDirMount(World world, String subPath, long capacity) {
        findCC();

        if (computerCraft_createSaveDirMount != null) {
            try {
                return (IWritableMount
                ) computerCraft_createSaveDirMount.invoke(null, world, subPath, capacity);
            } catch (Exception e) {
                // It failed
            }
        }
     ...
     * Use in conjuction with IComputerAccess.mount() or IComputerAccess.mountWritable()
     * to mount a resource folder onto a computers file system.<br> The files in this
     * mount will be a combination of files in the specified mod jar, and resource packs
     * that contain resources with the same domain and path.<br>
     * @param modClass A class in whose jar to look first for the resources to mount.
     *     Using your main mod class is recommended. eg: MyMod.class
     * @param domain The domain under which to look for resources. eg: "mymod"
     * @param subPath The domain under which to look for resources. eg:
     *     "mymod/lua/myfiles"
     * @return The mount, or null if it could be created for some reason. Use
     *     IComputerAccess.mount() or IComputerAccess.mountWritable()
     * to mount this on a Computers' file system.
     ...
     * @see dan200.computercraft.api.filesystem.IMount
     */
    public static IMount
    createResourceMount(Class modClass, String domain, String subPath) {
        findCC();

        if (computerCraft_createResourceMount != null) {
            try {
                return (IMount) computerCraft_createResourceMount.invoke(
                    null, modClass, domain, subPath
                );
            } catch (Exception e) {
                // It failed
            }
     ...
     * users should be able to craft Turtles with your new turtle. It is recommended to
     * call this during the load() method of your mod.
    ...
    /**
     * If there is a Computer or Turtle at a certain position in the world, get it's
     * bundled redstone output.
     * @see dan200.computercraft.api.redstone.IBundledRedstoneProvider
     * @return If there is a block capable of emitting bundled redstone at the location,
     *     it's signal (0-65535) will be returned.
     * If there is no block capable of emitting bundled redstone at the location, -1 will
     * be returned.
     */
    public static int
    getBundledRedstoneOutput(World world, int x, int y, int z, int side) {
        findCC();

        if (computerCraft_getDefaultBundledRedstoneOutput != null) {
            try {
                return (Integer) computerCraft_getDefaultBundledRedstoneOutput.invoke(
                    null, world, x, y, z, side
                );
            } catch (Exception e) {
                // It failed
            }
    ...
    // The functions below here are private, and are used to interface with the non-API
    // ComputerCraft classes. Reflection is used here so you can develop your mod without
    // decompiling ComputerCraft and including it in your solution, and so your mod won't
    // crash if ComputerCraft is installed.
        ...
        if (!ccSearched) {
            try {
                ...
                computerCraft_registerTurtleUpgrade = findCCMethod(
                    "registerTurtleUpgrade", new Class[] { ITurtleUpgrade.class }
                );
                ...
                computerCraft_registerMediaProvider = findCCMethod(
                    "registerMediaProvider", new Class[] { IMediaProvider.class }
                );
                computerCraft_registerPermissionProvider = findCCMethod(
    ...
    private static Method findCCMethod(String name, Class[] args) {
        try {
            ...
            System.out.println(
                "ComputerCraftAPI: ComputerCraft method " + name + " not found."
            );
            return null;
        }
    }
